...
let handler = async (m, { conn, text }) => {
  ...
  if (!who) throw 'Tag salah satu Onii-chan'
  let txt = text.replace('@' + who.split`@`[0], '').trim()
  if (isNaN(txt)) throw 'cuma angka onii-chan'
  let xp = parseInt(txt)
  let exp = xp
  ...
  if (exp > users[m.sender].exp) throw 'Gomenasai oni-chan Exp tidak mencukupi untuk mentransfer'
